# Baron, Stephan & Frahm 1996, Table13: volumes of the sensory trigeminal complex and of the tractus solitarii and funicular nuclei of the medulla oblongata.
# Frozen source: Baron_etal_1996_Table13_snapshot.csv, produced by the coordinate-based
...
final.dataframe <- data.frame(
  ...
  trigeminal_sensory_complex_mm3 = as.numeric(snapshot$TR),
  nucleus_tractus_solitarii_mm3 = as.numeric(snapshot$TSO),
  funicular_nuclei_mm3 = as.numeric(snapshot$FUN),
  nucleus_fascicularis_gracilis_mm3 = as.numeric(snapshot$FGR),
  nucleus_fascicularis_cuneatus_medialis_mm3 = as.numeric(snapshot$FCM),
  nucleus_fascicularis_cuneatus_externus_mm3 = as.numeric(snapshot$FCE),
  source_pdf_page = snapshot$source_pdf_page,
  stringsAsFactors = FALSE
)

required <- c("species_row", "Species_Baron1996", "trigeminal_sensory_complex_mm3", "nucleus_tractus_solitarii_mm3", "funicular_nuclei_mm3", "nucleus_fascicularis_gracilis_mm3", "nucleus_fascicularis_cuneatus_medialis_mm3", "nucleus_fascicularis_cuneatus_externus_mm3")
