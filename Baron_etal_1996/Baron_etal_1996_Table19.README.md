# Baron et al. 1996 <e2><80><94> Table19
...
ISBN **978-3-7643-5370-4**. Registry item `Baron_etal_1996_Table19`.

volumes of the vestibular nuclei of the medulla oblongata.

Printed on PDF pages 98-102; 150 rows, matching the species count the book gives for
these structures in its own Table 1 key (PDF pp. 264-265). Columns carried:
`vestibular_complex_mm3`, `nucleus_vestibularis_medialis_mm3`, `nucleus_vestibularis_inferior_mm3`, `nucleus_vestibularis_lateralis_mm3`, `nucleus_vestibularis_superior_mm3`.

## Reproducible pipeline

1. `baron_etal_1996 book Comparative Neurobiology.pdf`, pages 98-102, is the printed source.
2. `Baron_etal_1996_extract_snapshots.R` reads the PDF **word coordinates**
   (`pdftools::pdf_data`) and writes `Baron_etal_1996_Table19_snapshot.csv`. The book
...
3. `Baron_etal_1996_Table19.R` reads only the frozen snapshot and writes this local
   CSV plus `ISBN%3A9783764353704_Table19.tsv` in `__Public`.
...
tables (Table20-21) are derived and are deliberately not transcribed.
