# Baron, Stephan & Frahm 1996, Table22: volumes of the auditory nuclei of the medulla oblongata.
# Frozen source: Baron_etal_1996_Table22_snapshot.csv, produced by the coordinate-based
...
final.dataframe <- data.frame(
  ...
  auditory_nuclei_mm3 = as.numeric(snapshot$AUD),
  cochlear_nuclei_mm3 = as.numeric(snapshot$CON),
  nucleus_cochlearis_dorsalis_mm3 = as.numeric(snapshot$DCO),
  nucleus_cochlearis_ventralis_mm3 = as.numeric(snapshot$VCO),
  complexus_olivaris_superior_mm3 = as.numeric(snapshot$OLS),
  source_pdf_page = snapshot$source_pdf_page,
  stringsAsFactors = FALSE
)

required <- c("species_row", "Species_Baron1996", "auditory_nuclei_mm3", "cochlear_nuclei_mm3", "nucleus_cochlearis_dorsalis_mm3", "nucleus_cochlearis_ventralis_mm3", "complexus_olivaris_superior_mm3")
