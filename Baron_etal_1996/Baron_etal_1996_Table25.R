# Baron, Stephan & Frahm 1996, Table25: volumes of mesencephalic components. The MES column is the same measure as Table 10's mesencephalon and agrees with it to the printed rounding scale.
# Frozen source: Baron_etal_1996_Table25_snapshot.csv, produced by the coordinate-based
...
final.dataframe <- data.frame(
  ...
  tegmentum_mesencephali_mm3 = as.numeric(snapshot$MTG),
  tectum_mesencephali_mm3 = as.numeric(snapshot$MTC),
  colliculi_inferiores_mm3 = as.numeric(snapshot$INC),
  colliculi_superiores_mm3 = as.numeric(snapshot$SUC),
  source_pdf_page = snapshot$source_pdf_page,
  stringsAsFactors = FALSE
)

required <- c("species_row", "Species_Baron1996", "mesencephalon_mm3", "tegmentum_mesencephali_mm3", "tectum_mesencephali_mm3", "colliculi_inferiores_mm3", "colliculi_superiores_mm3")
