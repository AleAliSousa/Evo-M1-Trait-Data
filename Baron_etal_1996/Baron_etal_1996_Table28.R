# Baron, Stephan & Frahm 1996, Table28: volumes of the cerebellar nuclei. The CER column is the same measure as Table 10's cerebellum.
# Frozen source: Baron_etal_1996_Table28_snapshot.csv, produced by the coordinate-based
...
stopifnot(nrow(snapshot) == 18L, identical(snapshot$species_row, seq_len(18L)))
...
final.dataframe <- data.frame(
  ...
  total_cerebellar_nuclei_mm3 = as.numeric(snapshot$TCN),
  nucleus_medialis_cerebelli_mm3 = as.numeric(snapshot$MCN),
  nucleus_interpositus_cerebelli_mm3 = as.numeric(snapshot$ICN),
  nucleus_lateralis_cerebelli_mm3 = as.numeric(snapshot$LCN),
  source_pdf_page = snapshot$source_pdf_page,
  stringsAsFactors = FALSE
)

required <- c("species_row", "Species_Baron1996", "cerebellum_mm3", "total_cerebellar_nuclei_mm3", "nucleus_medialis_cerebelli_mm3", "nucleus_interpositus_cerebelli_mm3", "nucleus_lateralis_cerebelli_mm3")
