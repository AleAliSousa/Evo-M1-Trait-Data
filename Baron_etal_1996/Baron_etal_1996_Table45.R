# Baron, Stephan & Frahm 1996, Table45: Lutgemeier's (1962) subdivision of the "bicortex", reprinted in the book. Secondary. The four sub-region keys are printed as abbreviations only and are carried verbatim.
# Frozen source: Baron_etal_1996_Table45_snapshot.csv, produced by the coordinate-based
...
final.dataframe <- data.frame(
  ...
  RB_mm2 = as.numeric(snapshot$RB),
  TT_mm2 = as.numeric(snapshot$TT),
  CA_mm2 = as.numeric(snapshot$CA),
  FD_mm2 = as.numeric(snapshot$FD),
  source_pdf_page = snapshot$source_pdf_page,
  stringsAsFactors = FALSE
)

required <- c("species_row", "Species_Baron1996", "bicortex_mm2", "RB_mm2", "TT_mm2", "CA_mm2", "FD_mm2")
