# Baron et al. 1996 <e2><80><94> Table45
...
ISBN **978-3-7643-5370-4**. Registry item `Baron_etal_1996_Table45`.

Lutgemeier's (1962) subdivision of the "bicortex", reprinted in the book. Secondary. The four sub-region keys are printed as abbreviations only and are carried verbatim.

Printed on PDF pages 163; 6 rows, matching the species count the book gives for
these structures in its own Table 1 key (PDF pp. 264-265). Columns carried:
`bicortex_mm2`, `RB_mm2`, `TT_mm2`, `CA_mm2`, `FD_mm2`.

## Reproducible pipeline

1. `baron_etal_1996 book Comparative Neurobiology.pdf`, pages 163, is the printed source.
2. `Baron_etal_1996_extract_snapshots.R` reads the PDF **word coordinates**
   (`pdftools::pdf_data`) and writes `Baron_etal_1996_Table45_snapshot.csv`. The book
...
3. `Baron_etal_1996_Table45.R` reads only the frozen snapshot and writes this local
   CSV plus `ISBN%3A9783764353704_Table45.tsv` in `__Public`.
...
tables (Table46-47) are derived and are deliberately not transcribed.
