# Baron, Stephan & Frahm 1996, Table8: volumes of net brain (NET), ventricles (VENT) and rests (REST). Only the mm3 columns are printed measurements; the percentage columns are the book's own derivation against the standard brain volume and are carried for reference.
# Frozen source: Baron_etal_1996_Table8_snapshot.csv, produced by the coordinate-based
...
final.dataframe <- data.frame(
  ...
  n = as.numeric(snapshot$n),
  net_brain_mm3 = as.numeric(snapshot$NET_mm3),
  net_brain_pct_of_standard = as.numeric(snapshot$NET_pct),
  ventricles_mm3 = as.numeric(snapshot$VENT_mm3),
  ventricles_pct_of_standard = as.numeric(snapshot$VENT_pct),
  rests_mm3 = as.numeric(snapshot$REST_mm3),
  rests_pct_of_standard = as.numeric(snapshot$REST_pct),
  source_pdf_page = snapshot$source_pdf_page,
  stringsAsFactors = FALSE
)

required <- c("species_row", "Species_Baron1996", "net_brain_mm3", "ventricles_mm3", "rests_mm3")
