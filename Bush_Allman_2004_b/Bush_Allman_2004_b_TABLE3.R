...
##
## Input : Bush_Allman_2004_b_TABLE3_snapshot.csv
## Output: Bush_Allman_2004_b_TABLE3.csv
...
snap <- read.csv("Bush_Allman_2004_b_TABLE3_snapshot.csv", check.names = FALSE,
                 stringsAsFactors = FALSE, na.strings = c("", "NA", "n.a.", "-", "--"))

stopifnot(nrow(snap) == 12L)
num <- function(x) suppressWarnings(as.numeric(gsub(",", "", trimws(as.character(x)))))

clean <- data.frame(
  species                                   = trimws(snap$species),
  retinal_ganglion_cell_count               = num(snap$count),
  reference                                 = trimws(snap$reference),
  source                                    = "Bush_Allman_2004_b",
  stringsAsFactors = FALSE
)

stopifnot(nrow(clean) == 12L, !anyDuplicated(clean$species), all(nzchar(clean$species)))
