## Dunbar (1992), Table 1: Data for main variables (generic means)
## Neocortex size as a constraint on group size in primates.
## Journal of Human Evolution 22:469-493.
##
## Input : Dunbar__1992_Table1_snapshot.csv
## Output: Dunbar__1992_Table1.csv and DOI-coded public TSV
options(scipen=999)
.sp <- local({
...
snap <- read.csv("Dunbar__1992_Table1_snapshot.csv",check.names=FALSE,stringsAsFactors=FALSE,na.strings=c("","NA","-"))
stopifnot(nrow(snap)==37L,!anyDuplicated(snap$genus),all(nzchar(snap$genus)))
num <- setdiff(names(snap),"genus"); for(nm in num) snap[[nm]] <- suppressWarnings(as.numeric(snap[[nm]]))
clean <- snap; clean$source <- "Dunbar__1992_Table1"
