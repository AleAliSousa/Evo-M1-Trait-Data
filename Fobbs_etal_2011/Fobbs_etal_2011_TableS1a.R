# Fobbs_etal_2011_TableS1a.R
#
# Complete build pipeline for Fobbs & Johnson (2011), supplementary TableS1a:
...
#
# Source  : nyas_6036_sm_table1a.doc
# Helper  : Fobbs_etal_2011_build_snapshot.py
# Snapshot: Fobbs_etal_2011_TableS1a_snapshot.xlsx, sheet "TableS1a"
# Outputs : Fobbs_etal_2011_TableS1a.csv
...
source_doc <- "nyas_6036_sm_table1a.doc"
...
raw <- read_excel(snapshot_xlsx, sheet="TableS1a", skip=1, col_names=TRUE, col_types="text")
if (ncol(raw) != 10L) stop("Expected 10 columns; found ", ncol(raw), ".", call.=FALSE)
names(raw) <- c("diagnostic_category", "subcategory_1", "age_at_death", "gestational_age", "time_unit", "crown_to_rump_length_mm", "crown_to_heel_length_mm", "postnatal_survival", "sex", "plane_of_section")
...
# Remove printed section-label rows that do not describe a specimen.
final.dataframe <- final.dataframe %>% filter(!is.na(diagnostic_category), !is.na(plane_of_section))
