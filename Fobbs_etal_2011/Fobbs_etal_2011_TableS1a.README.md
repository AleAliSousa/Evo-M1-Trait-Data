# Fobbs et al. 2011 — Human normative-control and clinical fetal/infant brain specimens
...
Supplementary **Fobbs_etal_2011_TableS1a** — developmental neuropathology catalog: diagnostic category, gestational/postnatal age, body measurements, and sectioning plane for each human fetal/infant specimen.
...
- **Frozen source (printed/legacy, snapshot required):** `nyas_6036_sm_table1a.doc` is a legacy journal Word
  ...
  losslessly into `Fobbs_etal_2011_TableS1a_snapshot.xlsx` (sheet `TableS1a`); the snapshot is **not** rebuilt on every
  run (`rebuild_snapshot = FALSE` in the `.R` script) so the already-audited snapshot is protected
  from silent drift.
- **Reformat:** `Fobbs_etal_2011_TableS1a.R` reads the frozen snapshot by position, hard-checks the expected column
  ...
  column. Writes:
  - `Fobbs_etal_2011_TableS1a.csv` (analysis-ready, 432 catalog records)
  - `__Public/comparative-data/10.1111%2Fj.1749-6632.2011.06036.x_TableS1a.tsv` (public,
    DOI-encoded, already present — matches the registry's `Item encoded`)
- **Definitions:** `reference_tables/Fobbs_etal_2011_TableS1a_definitions.csv` (added now).
...
## Checks
- Analysis CSV = 432 catalog records (printed section-label and wholly-blank rows removed by the
