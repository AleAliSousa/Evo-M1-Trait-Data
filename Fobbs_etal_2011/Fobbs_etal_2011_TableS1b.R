# Fobbs_etal_2011_TableS1b.R
#
# Complete build pipeline for Fobbs & Johnson (2011), supplementary TableS1b:
...
#
# Source  : nyas_6036_sm_table1b.doc
# Helper  : Fobbs_etal_2011_build_snapshot.py
# Snapshot: Fobbs_etal_2011_TableS1b_snapshot.xlsx, sheet "TableS1b"
# Outputs : Fobbs_etal_2011_TableS1b.csv
...
source_doc <- "nyas_6036_sm_table1b.doc"
...
raw <- read_excel(snapshot_xlsx, sheet="TableS1b", skip=1, col_names=TRUE, col_types="text")
if (ncol(raw) != 9L) stop("Expected 9 columns; found ", ncol(raw), ".", call.=FALSE)
names(raw) <- c("species", "specifics", "experimental", "normative_controls", "age_at_death", "time_unit", "sex", "plane", "study")
...
final.dataframe <- final.dataframe %>% filter(!is.na(species), if_any(c(specifics, experimental, normative_controls, age_at_death, sex, plane), ~ !is.na(.x)))
