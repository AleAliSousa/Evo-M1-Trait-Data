# Fobbs et al. 2011 — Non-human comparative specimens (companion to Table S1a)
...
Supplementary **Fobbs_etal_2011_TableS1b** — non-human specimens catalogued alongside the human developmental collection: species, experimental/normative-control status, age, sex, and sectioning plane.
...
- **Frozen source (printed/legacy, snapshot required):** `nyas_6036_sm_table1b.doc` is a legacy journal Word
  ...
  losslessly into `Fobbs_etal_2011_TableS1b_snapshot.xlsx` (sheet `TableS1b`); the snapshot is **not** rebuilt on every
  run (`rebuild_snapshot = FALSE` in the `.R` script) so the already-audited snapshot is protected
  from silent drift.
- **Reformat:** `Fobbs_etal_2011_TableS1b.R` reads the frozen snapshot by position, hard-checks the expected column
  ...
  column. Writes:
  - `Fobbs_etal_2011_TableS1b.csv` (analysis-ready, 164 catalog records)
  - `__Public/comparative-data/10.1111%2Fj.1749-6632.2011.06036.x_TableS1b.tsv` (public,
    DOI-encoded, already present — matches the registry's `Item encoded`)
- **Definitions:** `reference_tables/Fobbs_etal_2011_TableS1b_definitions.csv` (added now).
...
## Checks
- Analysis CSV = 164 catalog records (printed section-label and wholly-blank rows removed by the
