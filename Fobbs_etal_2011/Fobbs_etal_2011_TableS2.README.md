# Fobbs et al. 2011 — Comparative-anatomy brain sections catalog
...
Supplementary **Fobbs_etal_2011_TableS2** — animal number, common name, brain part, stain(s), sectioning plane, section thickness, and slide/section counts for the comparative-anatomy collection.
...
- **Frozen source (printed/legacy, snapshot required):** `nyas_6036_sm_table2.doc` is a legacy journal Word
  ...
  losslessly into `Fobbs_etal_2011_TableS2_snapshot.xlsx` (sheet `TableS2`); the snapshot is **not** rebuilt on every
  run (`rebuild_snapshot = FALSE` in the `.R` script) so the already-audited snapshot is protected
  from silent drift.
- **Reformat:** `Fobbs_etal_2011_TableS2.R` reads the frozen snapshot by position, hard-checks the expected column
  ...
  column. Writes:
  - `Fobbs_etal_2011_TableS2.csv` (analysis-ready, 407 catalog records)
  - `__Public/comparative-data/10.1111%2Fj.1749-6632.2011.06036.x_TableS2.tsv` (public,
    DOI-encoded, already present — matches the registry's `Item encoded`)
- **Definitions:** `reference_tables/Fobbs_etal_2011_TableS2_definitions.csv` (added now).
...
## Checks
- Analysis CSV = 407 catalog records (printed section-label and wholly-blank rows removed by the
