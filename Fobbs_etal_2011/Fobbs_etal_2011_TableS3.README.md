# Fobbs et al. 2011 — Non-mammalian (fish/elasmobranch) brain sections catalog
...
Supplementary **Fobbs_etal_2011_TableS3** — animal number, common and scientific name, sectioning plane, section count, and slide size for the non-mammalian comparative collection.
...
- **Frozen source (printed/legacy, snapshot required):** `nyas_6036_sm_table3.doc` is a legacy journal Word
  ...
  losslessly into `Fobbs_etal_2011_TableS3_snapshot.xlsx` (sheet `TableS3`); the snapshot is **not** rebuilt on every
  run (`rebuild_snapshot = FALSE` in the `.R` script) so the already-audited snapshot is protected
  from silent drift.
- **Reformat:** `Fobbs_etal_2011_TableS3.R` reads the frozen snapshot by position, hard-checks the expected column
  ...
  column. Writes:
  - `Fobbs_etal_2011_TableS3.csv` (analysis-ready, 176 catalog records)
  - `__Public/comparative-data/10.1111%2Fj.1749-6632.2011.06036.x_TableS3.tsv` (public,
    DOI-encoded, already present — matches the registry's `Item encoded`)
- **Definitions:** `reference_tables/Fobbs_etal_2011_TableS3_definitions.csv` (added now).
...
## Checks
- Analysis CSV = 176 catalog records (printed section-label and wholly-blank rows removed by the
