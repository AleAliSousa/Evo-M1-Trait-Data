# Fobbs_etal_2011_TableS4.R
#
# Complete build pipeline for Fobbs & Johnson (2011), supplementary TableS4:
...
#
# Source  : nyas_6036_sm_table4.doc
# Helper  : Fobbs_etal_2011_build_snapshot.py
# Snapshot: Fobbs_etal_2011_TableS4_snapshot.xlsx, sheet "TableS4"
# Outputs : Fobbs_etal_2011_TableS4.csv
...
source_doc <- "nyas_6036_sm_table4.doc"
...
raw <- read_excel(snapshot_xlsx, sheet="TableS4", skip=1, col_names=TRUE, col_types="text")
if (ncol(raw) != 6L) stop("Expected 6 columns; found ", ncol(raw), ".", call.=FALSE)
names(raw) <- c("box_number", "specimens", "plane", "stain", "clade", "detail")
...
# Clade headings occur as source rows with no box number. Carry the printed heading
# forward while retaining every catalog row. Group labels themselves are excluded.
final.dataframe <- final.dataframe %>%
  mutate(collection_group = if_else(is.na(box_number) & !is.na(specimens) & is.na(plane) & is.na(stain), specimens, NA_character_)) %>%
  tidyr::fill(collection_group) %>%
  filter(!is.na(box_number))
