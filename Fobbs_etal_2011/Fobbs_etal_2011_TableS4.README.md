# Fobbs et al. 2011 — Storage-box inventory of brain sections
...
Supplementary **Fobbs_etal_2011_TableS4** — box-level inventory (box number, specimen description, plane, stain, clade, free-text detail) for boxed comparative material, mostly primate.
...
- **Frozen source (printed/legacy, snapshot required):** `nyas_6036_sm_table4.doc` is a legacy journal Word
  ...
  losslessly into `Fobbs_etal_2011_TableS4_snapshot.xlsx` (sheet `TableS4`); the snapshot is **not** rebuilt on every
  run (`rebuild_snapshot = FALSE` in the `.R` script) so the already-audited snapshot is protected
  from silent drift.
- **Reformat:** `Fobbs_etal_2011_TableS4.R` reads the frozen snapshot by position, hard-checks the expected column
  ...
  column. Writes:
  - `Fobbs_etal_2011_TableS4.csv` (analysis-ready, 283 catalog records)
  - `__Public/comparative-data/10.1111%2Fj.1749-6632.2011.06036.x_TableS4.tsv` (public,
    DOI-encoded, already present — matches the registry's `Item encoded`)
- **Definitions:** `reference_tables/Fobbs_etal_2011_TableS4_definitions.csv` (added now).
...
## Checks
- Analysis CSV = 283 catalog records (printed section-label and wholly-blank rows removed by the
