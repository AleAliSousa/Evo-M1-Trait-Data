# Fobbs et al. 2011 — Non-mammal brain sections catalog (companion to Table S5a)
...
Supplementary **Fobbs_etal_2011_TableS5b** — animal number, common name, stain(s), plane, non-mammal clade label, slide count, and free-text comments.
...
- **Frozen source (printed/legacy, snapshot required):** `nyas_6036_sm_table5b.doc` is a legacy journal Word
  ...
  losslessly into `Fobbs_etal_2011_TableS5b_snapshot.xlsx` (sheet `TableS5b`); the snapshot is **not** rebuilt on every
  run (`rebuild_snapshot = FALSE` in the `.R` script) so the already-audited snapshot is protected
  from silent drift.
- **Reformat:** `Fobbs_etal_2011_TableS5b.R` reads the frozen snapshot by position, hard-checks the expected column
  ...
  column. Writes:
  - `Fobbs_etal_2011_TableS5b.csv` (analysis-ready, 101 catalog records)
  - `__Public/comparative-data/10.1111%2Fj.1749-6632.2011.06036.x_TableS5b.tsv` (public,
    DOI-encoded, already present — matches the registry's `Item encoded`)
- **Definitions:** `reference_tables/Fobbs_etal_2011_TableS5b_definitions.csv` (added now).
...
## Checks
- Analysis CSV = 101 catalog records (printed section-label and wholly-blank rows removed by the
