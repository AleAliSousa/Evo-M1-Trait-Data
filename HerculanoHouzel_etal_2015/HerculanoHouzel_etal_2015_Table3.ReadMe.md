Source

Download Publication pdf from Source: 
https://karger.com/bbe/article-pdf/86/3-4/145/2264694/000437413.pdf

--> Primary or Equivalent

Open in Adobe Acrobat Pro. Export a PDF > Microsoft Excel > Microsoft Excel Workbook 

Copy and paste Table 3 and manually format it

HerculanoHouzel_etal_2015_Table3.xlsx

--> Data readable   

HerculanoHouzel_etal_2015_Table3.csv <-- USE THIS

--> Online database

Added TSV copy named with DOI to https://github.com/r03ert0/comparative-data

10.1159%2F000437413_table3.tsv <-- ONLINE COPY