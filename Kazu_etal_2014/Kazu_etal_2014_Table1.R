## Kazu et al. 2014 — Table 1 (Artiodactyla cell counts)
## doi:10.3389/fnana.2014.00128
##
## PRINTED SOURCE -> the frozen copy is Kazu_etal_2014_Table1_snapshot.xlsx (sheet "Table1"),
## captured from the article PDF by Kazu_etal_2014_extract_snapshot.py. This script reads
## ONLY that snapshot (golden rule: all cleaning happens here, never in the frozen copy).
##
## The snapshot is the table AS PRINTED IN 2014. It is NOT the 2015 corrigendum
## (doi:10.3389/fnana.2015.00039), which is not in this folder; several 2014 cells are
## internally inconsistent and were later corrected. The checks in section 6 below
## reproduce those inconsistencies rather than hiding them, and every affected species
## carries a `consistency_flags` string. Read Kazu_etal_2014_Table1.README.md before using
## these numbers for anything: the corrigendum values already reach the merge through
## HerculanoHouzel_etal_2015 Tables 1-5, at full precision.
##
...
term_of <- function(label) {
  ...
  stopifnot(length(p) == 2, p[1] %in% names(MEASURE), p[2] %in% names(STRUCT))
  paste0(STRUCT[[p[2]]], "_", MEASURE[[p[1]]])
}
...
for (s in seq_along(species_printed)) {
  ...
  row <- data.frame(
    ...
    Species_Kazu2014 = printed,                 # printed name kept verbatim (invariant 3)
...
cat(sprintf("Kazu Table 1: %d species x %d columns; %d rows with parse flags, %d with consistency flags\n",
            nrow(out), ncol(out), sum(!is.na(out$parse_flags)), sum(!is.na(out$consistency_flags))))
for (i in seq_len(nrow(out)))
  if (!is.na(out$consistency_flags[i]))
    cat("  ", out$Species_Kazu2014[i], ": ", out$consistency_flags[i], "\n", sep = "")
