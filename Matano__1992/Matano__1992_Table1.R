...
#
#   Table 1. Body weight, volumes of the inferior olivary nuclei and eco-ethological
#   characteristics in Scandentia and prosimians (19 species: 2 Scandentia + 17 prosimians).
#
...
#
# Input
#   Matano__1992_Table1_snapshot.csv   journal-faithful transcription of the printed
#     table (the data rows are scanned images with no text layer, so this hand-read
#     snapshot is the frozen source).
#
# Outputs
#   Matano__1992_Table1.csv                          19 species
#   10.1537%2Fase1911.100.69_Table1.tsv              in __Public/comparative-data/
#
...
out <- data.frame(
  ...
  # Table 1 is the only table that mixes two blocks: 2 Scandentia, then 17 prosimians.
  group          = ifelse(snap$Species %in% c("Tupaia glis", "Urogale everetti"),
                          "Scandentia", "Prosimian"),
  n              = snap$Specimens_n,
...
stopifnot(nrow(out) == 19L, max(resid) <= 0.06)
