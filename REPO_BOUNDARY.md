...
**Canonical definition of what belongs in which of the two repositories, updated 2026-08-19.**
...
1. **The six publishable audits.** `_triage_comparison_inputs.csv` finds 6 of 42 check folders whose
   ...
   private because it must be or because nobody has looked.
...
5. **`specimen_registry/` in the private repo** (13 files, committed 2026-08-19 on branch
   `codex/specimen-registry-split`) overlaps `_keys/specimen_crosswalk/` in the public repo, and its
   own `specimen_crosswalk_combined_pre_split_2026-08-19.csv` is named as if the split were already
   agreed. Specimen identity is normally public evidence — the part of the crosswalk built from
   published specimen tables belongs in `_keys/`; only the collection records that are not ours to
   publish belong here. Until that line is drawn per file, the same crosswalk exists on both sides,
   which is exactly what invariant I4 forbids.

6. **`SFI dataset/` in the private repo** (OCR QC pages, strips, and reviewer JSON) predates the
