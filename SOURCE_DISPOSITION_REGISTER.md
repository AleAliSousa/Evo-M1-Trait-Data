...
| Medina-González 2026, terrestrial-mammal joint excursion | `BUILT` (2026-09) | Zenodo record `10.5281/zenodo.15425733` remains restricted and still returns no files to unauthenticated clients -- but 3 of the paper's supplementary files (source list, locomotor-habit classification, joint-excursion data) were obtained directly from the published article page (onlinelibrary.wiley.com/doi/10.1002/jez.70069) and built as `MedinaGonzález__2026_SupplementaryFile1/2/3`, wired into `__merging_behaviour`. | Registered in `__ReadMe.xlsx` rows 258-260 by the owner (all 3 files as full items, `Data role = both`). | n/a -- resolved via the Wiley supplement rather than Zenodo access. | [`MedinaGonzález__2026.README.md`](MedinaGonzález__2026/MedinaGonzález__2026.README.md) |
...
| Halley & Krubitzer 2019, Figure 1 thalamus-neocortex plot | `DOCUMENTED SKIP` | All 39 points reconstruct from upstream primary tables already held: 37 Stephan et al. 1981 rows plus the two Campos & Welker 1976 rodents. Digitization would create rounded secondary duplicates. | Use the upstream source tables and retain the 39-row source audit; do not create a public Halley TSV or measurement team. | A citable machine-readable source supplies genuinely new measurements not present in Stephan or Campos. | [`Halley_Krubitzer_2019_Figure1.README.md`](Halley_Krubitzer_2019/Halley_Krubitzer_2019_Figure1.README.md) |
...
| Changizi & He 2005 Table 1 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Conceptual examples of hierarchical relationships between nodes and structures across network types; no species-level intelligence measurements. Retain as potentially useful network-complexity metadata if that becomes in scope. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Changizi & He 2005 Table 2 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Conceptual examples of lower- and higher-level behavioral relationships across systems; no species rows or directly mergeable comparative measures. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Deaner et al. 2006 Table 2 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Predictive-ability summary for eight hierarchical Bayesian model variants, not taxon-level cognitive measurements. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Deaner et al. 2006 Table 3 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Pair-wise probabilities comparing mean global cognitive variables among broad taxonomic groups, not raw or species-average observations. Higher global-variable values mean worse performance in this table. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Deaner et al. 2007 Table 1 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Statistical relationships between global cognitive ability and brain/body measures. The taxon-level cognition values come from Deaner et al. 2006 and are not reproduced here. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Reader et al. 2011 Table 1 | item + comparative-intelligence target | `DOCUMENTED SKIP` | PCA loadings for five cognitive measures and general-intelligence component gS1 across 62 species; species-level scores are not printed in the table. Use the accompanying digital species dataset for species values. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Reader et al. 2011 Table 2 | item + comparative-intelligence target | `DOCUMENTED SKIP` | PCA loadings for eight cognitive and socio-ecological measures and components gS2/component 2 across 44 species; not a species-level value table. Use the accompanying digital species dataset. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Reader et al. 2011 Table 3 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Correlation matrix for research-effort-corrected cognitive and socio-ecological measures. Useful relationship summary, but it does not supply raw or species-average rows. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Reader et al. 2011 Table 4 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Regression summary relating gS1 to four brain-size measures, including across-species and independent-contrast analyses; no species-level observations. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Shultz & Dunbar 2010 Table 1 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Presence/absence coverage matrix showing which primate genera occur in eight executive-function tasks. Useful sampling metadata, but not performance values. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Shultz & Dunbar 2010 Table 2 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Univariate associations between executive-task performance and brain-component measures; model summary across tasks rather than species observations. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Shultz & Dunbar 2010 Table 3 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Best multivariate models relating executive performance to hippocampus, neocortex ratio, and other brain measures; model summary rather than species observations. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Shultz & Dunbar 2010 Table 4 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Task-specific correlations between performance and brain-size measures; comparative task-level summary, not species rows. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Shultz & Dunbar 2010 Table 5 | item + comparative-intelligence target | `DOCUMENTED SKIP` | Task-by-task BIC comparison of alternative brain-measure models; model-comparison statistics rather than species values. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
| Reader & Laland 2002 Table 1 | item + comparative-intelligence target | `DOCUMENTED SKIP` | P values from a limited field-only reanalysis of innovation, social learning, tool use, and executive brain ratio. Raw species values were available from the authors but are not printed; do not reconstruct them from the summary statistics. | `__ReadMe.xlsx` → `SOURCE_DISPOSITION` |
...
| 2026-09-05 | Medina-González 2026 changed from `HOLD` to `BUILT`: 3 supplementary files obtained from the Wiley online article (onlinelibrary.wiley.com/doi/10.1002/jez.70069) rather than the still-restricted Zenodo deposit; built as MedinaGonzalez__2026_SupplementaryFile1/2/3 (all 3 registered as full items, Data role = both), wired into __merging_behaviour. Registered in __ReadMe.xlsx rows 258-260 by the owner. |
| 2026-08-20 | Added 15 item-level comparative-intelligence decisions from the `__ReadMe.xlsx` `SOURCE_DISPOSITION` worksheet: Changizi & He 2005 Tables 1–2; Deaner et al. 2006 Tables 2–3; Deaner et al. 2007 Table 1; Reader et al. 2011 Tables 1–4; Shultz & Dunbar 2010 Tables 1–5; and Reader & Laland 2002 Table 1. All are retained as `DOCUMENTED SKIP` for the raw/species-average target, with their conceptual, coverage, correlation, or model-summary value recorded. |
| 2026-08-24 | Halley & Krubitzer 2019 Figure 1 set to `DOCUMENTED SKIP`: all 39 points were reconstructed from Stephan 1981 (37) and Campos & Welker 1976 (2); no plot digitization or public TSV. |
