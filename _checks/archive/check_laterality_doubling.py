...
What it checks
  ...
  2. Every registered column OF A MERGED SOURCE resolves to a standardized term. A guard
     registered for a source that is in no merge yet has nothing to resolve against; it is
     reported PEND, not FAIL.
...
# A registered column with no standardized term is one of two very different things, and only one
# of them is a defect:
...
#     laterality note says so). Pre-registering a guard must not be punished as an error, so it is
#     reported as PEND: informational, and not a failure (see the header, "Exit").
merged_refs = {ref for ref, _ in tmap}
unmapped = [r for r in lat if (r["Reference"], r["Original_Term"]) not in tmap]
drifted = [r for r in unmapped if r["Reference"] in merged_refs]
guard_only = [r for r in unmapped if r["Reference"] not in merged_refs]
report(not drifted, "every registered column of a merged source has a standardized term",
       "; ".join(f"{r['Reference']}:{r['Original_Term']}" for r in drifted))
if guard_only:
    lbl = ("laterality guard pre-registered for "
           + ", ".join(sorted({r["Reference"] for r in guard_only}))
           + ", which is in no merge yet")
    print(f"  [PEND] {lbl} -- "
          + "; ".join(f"{r['Reference']}:{r['Original_Term']}" for r in guard_only))
    pending.append(lbl)
